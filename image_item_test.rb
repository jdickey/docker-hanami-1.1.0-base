...
describe ImageItem do
  let(:default_hanami_version) { '1.1.1' }
  let(:default_image_version) { '9.99.0' }
  let(:default_ruby_version) { '2.5.0' }
  let(:valid_params) do
    {
      ...
      tags: ['2.5-stretch-slim-no-hm-qt', 'stretch-slim-no-hm-qt']
    }
  end
  ...
  describe 'has reader methods for' do
    ...
    it 'hanami_version' do
      obj = ImageItem.new(valid_params)
      expect(obj.hanami_version).must_equal default_hanami_version
    end

    describe 'hm?, such that when initialised with' do
      it '"hm", the #hm reader returns true' do
        valid_params[:hm] = 'hm'
        expect(ImageItem.new(valid_params).hm?).must_equal true
      end

      it '"no-hm", the #hm reader returns false' do
        valid_params[:hm] = 'no-hm'
        expect(ImageItem.new(valid_params).hm?).must_equal false
      end
    end # describe 'hm?, such that when initialised with'
    ...
    describe 'tags' do
      ...
      it 'when initialised with a single string, returns an array' do
        ...
        obj = ImageItem.new(valid_params)
        expect(obj.tags).must_equal ['some-other-tag']
      end
    end # 'tags'
  ...
  describe '#build_tag_name' do
    it 'returns the correct build tag for the initialised parameters' do
      parts = [:ruby_version, :base_os, :hm, :qt].map do |part|
        valid_params[part]
      end
      expected = parts.join('-')
